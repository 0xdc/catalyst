...
genkernel_compile() {
	...
	GK_ARGS=(
		...
		--minkernpackage=/tmp/kerncache/${kname}-kernel-initrd-${clst_version_stamp}.tar.bz2 kernel
	)
	...
	kernel_release="$(make --no-print-directory -C /usr/src/linux kernelrelease)"
	dracut --add dmsquash-live --force "/boot/initramfs-${kernel_release}.img" "${kernel_release}" || exit 2
	bunzip2 "/tmp/kerncache/${kname}-kernel-initrd-${clst_version_stamp}.tar.bz2"
	tar --append --file "/tmp/kerncache/${kname}-kernel-initrd-${clst_version_stamp}.tar" --directory "/boot" "initramfs-${kernel_release}.img"
	bzip2 "/tmp/kerncache/${kname}-kernel-initrd-${clst_version_stamp}.tar"
}
